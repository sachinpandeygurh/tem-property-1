...
const AdminPanel: React.FC = () => {
  ...
  const fetchTempUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://65.0.109.54:5000/api/v1/temp/users');
  ...
  const fetchTempProperties = async (page = 1, filterParams = {}) => {
    try {
      setLoading(true);
      const response = await axios.get('http://65.0.109.54:5000/api/v1/temp/properties', {
        params: {
          page: page,
          limit: 10,
          ...filterParams
  ...
  const handleDeleteTempUser = async (userId: string) => {
    if (window.confirm('Are you sure you want to delete this temporary user?')) {
      try {
        setLoading(true);
        await axios.delete(`http://65.0.109.54:5000/api/v1/temp/users/${userId}`);
        fetchTempUsers();
  ...
  const handleDeleteTempProperty = async (propertyId: string) => {
    if (window.confirm('Are you sure you want to delete this temporary property?')) {
      try {
        setLoading(true);
        await axios.delete(`http://65.0.109.54:5000/api/v1/temp/properties/${propertyId}`);
        fetchTempProperties();
